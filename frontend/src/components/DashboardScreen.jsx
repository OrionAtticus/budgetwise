...
import { listGoals, createGoal, contributeToGoal } from '../api/goals.js';
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 24 }}>
            {familyData.sharedGoals.map((g) => (
              <div key={g.id} style={{ background: '#fff', border: '1px solid #e8e4df', borderRadius: 16, padding: 20 }}>
                ...
                <div style={{ fontSize: 14, fontWeight: 600 }}>{g.name}</div>
                <div style={{ fontSize: 11, color: '#a8a29e' }}>Target ${g.targetAmount.toLocaleString()}{g.deadline ? ` · ${new Date(g.deadline).toLocaleDateString('en-US', { month: 'short', year: 'numeric' })}` : ''}</div>
                <div style={{ fontFamily: "'DM Mono',monospace", fontSize: 18, color: '#3d6b52', marginTop: 4 }}>${g.currentAmount.toLocaleString()}</div>
                <div style={{ fontSize: 11, color: '#57534e', marginTop: 2 }}>{g.progressPercent}% complete</div>
                <div style={{ height: 6, background: '#f2f0ed', borderRadius: 3, overflow: 'hidden', marginTop: 8 }}>
                  <div style={{ height: '100%', width: `${g.progressPercent}%`, background: '#3d6b52', borderRadius: 3 }} />
                </div>
              </div>
            ))}
